...
def perform_take_action_for_report(report: OceanHazardReport, acting_user: CustomUser | None = None):
    """Helper to perform take-action notifications for a report.

    This encapsulates sending emails and voice alerts to team members. It's safe
    to call from other views (e.g., automatic AI monitoring flow).
    ...
    try:
        # Prefer team members under the acting user (authority) if provided
        if acting_user:
            team_members = SubAuthorityTeamMember.objects.filter(sub_authority=acting_user, is_active=True)
        else:
            team_members = SubAuthorityTeamMember.objects.filter(district__iexact=(report.district or ''), is_active=True)

        results['team_members_count'] = team_members.count()

        if not team_members.exists():
            logger.info(f"No team members found for take action on report {report.report_id}")
            return results
        ...
        base_url = ''
        try:
            base_url = ''  # can't build request URL here; leave blank or use settings
        except Exception:
            base_url = ''
        ...
        for member in team_members:
            # Send email
            try:
                email_subject = f"URGENT: Hazard Report #{report.report_id} requires action"
                email_body = f"Report ID: {report.report_id}\nType: {report.get_hazard_type_display()}\nLocation: {report.city}, {report.district}, {report.state}\nDescription: {report.description[:200]}\n"
                EmailService.send_email(
                    subject=email_subject,
                    plain_text=email_body,
                    to_email=member.email
                )
                results['email_results'].append({'member_id': member.id, 'email': member.email, 'status': 'sent'})
                results['successful_emails'] += 1
            except Exception as e:
                logger.error(f"Failed to send email to {member.email}: {e}")
                results['email_results'].append({'member_id': member.id, 'email': member.email, 'status': 'failed', 'error': str(e)})

            # Make Twilio voice call if possible
            if twilio_client and member.phone_number:
                try:
                    twiml_content = f"<?xml version=\"1.0\" encoding=\"UTF-8\"?><Response><Say voice=\"alice\">Urgent hazard report {report.report_id} at {report.city}, {report.district}. Please check your email for details.</Say></Response>"
                    call = twilio_client.calls.create(
                        to=member.phone_number if member.phone_number.startswith('+') else f"+91{member.phone_number}",
                        from_=getattr(settings, 'TWILIO_PHONE_NUMBER', None),
                        twiml=twiml_content
                    )
                    results['call_results'].append({'member_id': member.id, 'phone': member.phone_number, 'call_sid': call.sid, 'status': 'initiated'})
                    results['successful_calls'] += 1
                except Exception as e:
                    logger.error(f"Failed to call {member.phone_number}: {e}")
                    results['call_results'].append({'member_id': member.id, 'phone': member.phone_number, 'status': 'failed', 'error': str(e)})

        return results

    except Exception as e:
        logger.exception(f"Error in perform_take_action_for_report: {e}")
        return results
